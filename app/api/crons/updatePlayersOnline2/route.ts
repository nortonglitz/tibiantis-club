...
export async function GET() {
    ...
    try {
        const playersOnline2 = await prisma.character.findMany({
            where: { online: true },
            orderBy: { level: 'desc' },
            skip: 100,
            take: 200
        ...
        const playersOnline2UpdatePromises = playersOnline2.map(async ({ displayName, id }) => {
        ...
        await Promise.all(playersOnline2UpdatePromises)

        return Response.json({
            message: "300/700 players online updated."
        }, { status: 200 })
