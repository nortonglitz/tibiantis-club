...
export async function GET() {
    ...
    try {
        const playersOnline4 = await prisma.character.findMany({
            where: { online: true },
            orderBy: { level: 'desc' },
            skip: 600,
            take: 200
        ...
        const playersOnline4UpdatePromises = playersOnline4.map(async ({ displayName, id }) => {
        ...
        await Promise.all(playersOnline4UpdatePromises)

        return Response.json({
            message: "800/800 players online updated."
        }, { status: 200 })
