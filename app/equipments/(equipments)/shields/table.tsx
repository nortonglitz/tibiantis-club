...
import shieldsData from '../../../../prisma/seeds/shields'

import { FaSortAlphaDown, FaSortAlphaUp, FaSortNumericDown, FaSortNumericUp } from "react-icons/fa"

type Field = "name" | "def" | "weight"
type Order = "asc" | "desc"

const Table: React.FC = () => {

    const [shields, setShields] = useState([...shieldsData].sort((a, b) => !a.def ? 1 : !b.def ? -1 : b.def - a.def))
    ...
    const handleSort = (field: Field) => {
        if (field === 'name') {
            if (sortProps.order === 'desc') {
                setShields([...shields].sort((a, b) => a.name.localeCompare(b.name)))
                setSortProps({ field: 'name', order: 'asc' })
            } else {
                setShields([...shields].sort((a, b) => b.name.localeCompare(a.name)))
        ...
        if (field === 'def') {
            if (sortProps.order === 'desc') {
                setShields([...shields].sort((a, b) => !a.def ? -1 : !b.def ? 1 : a.def - b.def))
                setSortProps({ field: 'def', order: 'asc' })
            } else {
                setShields([...shields].sort((a, b) => !a.def ? 1 : !b.def ? -1 : b.def - a.def))
        ...
        if (field === 'weight') {
            if (sortProps.order === 'desc') {
                setShields([...shields].sort((a, b) => a.weight - b.weight))
                setSortProps({ field: 'weight', order: 'asc' })
            } else {
                setShields([...shields].sort((a, b) => b.weight - a.weight))
                ...
                <table className="relative text-center w-full">
                    <thead className="top-0 sticky z-10">
                        ...
                        >
                            ...
                            <th className="min-w-[100px] cursor-pointer" scope="col" onClick={() => handleSort('def')}>
                                <div className="w-fit relative m-auto [&>svg]:hidden sm:[&>svg]:block [&>svg]:absolute [&>svg]:top-[0.1rem] [&>svg]:-right-6">
                                    Def
                    ...
                    <tbody>
                        {shields.map(({ weight, name, imageSrc, def }, i) => (
                            <tr
                                key={i}
                                className="
                                        [&>td]:py-2
