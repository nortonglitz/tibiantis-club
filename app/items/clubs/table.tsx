...
import clubsData from '../../../prisma/seeds/clubs'
...
const Table: React.FC = () => {
    ...
    const [clubs, setClubs] = useState([...clubsData].sort((a, b) => a.name.localeCompare(b.name)))
    ...
    const handleSort = (field: Field) => {
        if (field === 'name') {
            if (sortProps.order === 'desc') {
                ...
                setClubs([...clubs].sort((a, b) => b.name.localeCompare(a.name)))
        ...
        if (field === 'atk') {
            if (sortProps.order === 'desc') {
                setClubs([...clubs].sort((a, b) => a.atk - b.atk))
                setSortProps({ field: 'atk', order: 'asc' })
            } else {
                setClubs([...clubs].sort((a, b) => b.atk - a.atk))
        ...
        if (field === 'def') {
            if (sortProps.order === 'desc') {
                setClubs([...clubs].sort((a, b) => a.def - b.def))
                setSortProps({ field: 'def', order: 'asc' })
            } else {
                setClubs([...clubs].sort((a, b) => b.def - a.def))
        ...
        if (field === 'weight') {
            if (sortProps.order === 'desc') {
                setClubs([...clubs].sort((a, b) => a.weight - b.weight))
                setSortProps({ field: 'weight', order: 'asc' })
            } else {
                setClubs([...clubs].sort((a, b) => b.weight - a.weight))
        ...
        if (field === 'twoHanded') {
            if (sortProps.order === 'desc') {
                setClubs([...clubs].sort((a, b) => (a.twoHanded === b.twoHanded) ? 0 : a.twoHanded ? -1 : 1))
                setSortProps({ field: 'twoHanded', order: 'asc' })
            } else {
                setClubs([...clubs].sort((a, b) => (b.twoHanded === a.twoHanded) ? 0 : b.twoHanded ? -1 : 1))
        ...
        <div className="px-3 pt-1 bg-stone-800 rounded-3xl border border-stone-200/10 w-full sm:w-fit">
            <div className="h-[80vh] w-full sm:w-[75vw] overflow-y-auto rounded-xl">
                <table className="relative text-center w-full">
                    ...
                    <tbody>
                        {clubs.map(({ weight, name, imageSrc, atk, def, twoHanded }, i) => (
                            <tr
                                key={i}
                                className="
                                        [&>td]:py-2
